...
1.  Job & CronJobs 基础操作与概念解析；
2.  DaemonSet 基础操作与概念解析。
...
![avatar](https://images.gitbook.cn/Fo9k3bh5T3hAJxqA-3hFXVRkYF70)
...
![avatar](https://images.gitbook.cn/FsfyKX4pZ6ltGRES3Ma1GoCcb-64)
...
![avatar](https://images.gitbook.cn/Fv2wyMUcvFwpxCSrNUDdBEP4LeE9)
...
![avatar](https://images.gitbook.cn/FmV-DIGuMjGLQS12FhNgSIpi8MsJ)
...
![avatar](https://images.gitbook.cn/Fq8EvBaQV8x_2y4-MDw2kob3hGsQ)

#### 并行 Job 的编排文件

下面再来看第二个例子：

![avatar](https://images.gitbook.cn/Fv2O65ZDO--pHLz8xicotfl9_zZw)

#### 并行 Job 的创建及运行验证

这个例子就是指刚才的并行运行 Job 创建之后，可以看到有第二个并行的 Job。

![avatar](https://images.gitbook.cn/Fk5BhN8oUJVYijYcFib7rfe0vVRx)

现在已经有两个 Pod 正在 running，可以看到它大概执行了快到 30s

![avatar](https://images.gitbook.cn/FtXVmf5BjO8BRcc5LuhwXksXOFRD)

30s 之后它应该会起第二个。

![avatar](https://images.gitbook.cn/FuMQjONTy68OVSKCjTd6iyPUXhdt)
...
![avatar](https://images.gitbook.cn/Fh_xD3hXSn3EwNuY6pLWrmxxB2VB)

#### Cronjob 的编排文件

下面来看第三个例子 —— CronJob。 CronJob 是每分钟执行一次，每次一个 job。

![avatar](https://images.gitbook.cn/FhwqFsIjlfL4aUAAEiE0uHaL2Sb1)

#### Cronjob 的创建及运行验证

如下图 CronJob 已经创建了，可以通过 get cronjob 来看到当前有一个 CronJob，这个时候再来看 jobs，由于它是每分钟执行一次，所以得稍微等一下。

![avatar](https://images.gitbook.cn/FjIIXBz04a7wvbYbD5plUsy7tRCf)
...
![avatar](https://images.gitbook.cn/Ftwts5NgymiBDZRydR_CIcde835x)
...
![avatar](https://images.gitbook.cn/FlUJ1-4Ofde5w74sDvQ5OrEL66Rb)
...
![avatar](https://images.gitbook.cn/FtfcJwcyKLYwjGBjY9qHBGSr7SSA)
...
![avatar](https://images.gitbook.cn/Fn0gBsJY1Ugr9_M7kNFjcKV3O-0D)
...
![avatar](https://images.gitbook.cn/FukQvePjfaEKTrUJeuGIOznO4Qf5)
...
![avatar](https://images.gitbook.cn/FgVuatnFylUD_LRK6upX1PxBbwIQ)
...
![avatar](https://images.gitbook.cn/FvUc47P919bgEmefYt1kzYntI2ax)
...
![avatar](https://images.gitbook.cn/FvwwDrdF39A-IWcrKl5pQZBtEdpg)

从下图中可以看到，一共有 4 个 pod 被创建出来。为什么是 4 个 pod呢？因为只有 4 个节点，所以每个节点上都会运行一个对应的 pod。

![avatar](https://images.gitbook.cn/FtUY1TTz--z4l7YMXZEbXZ8yDSNn)

#### DaemonSet 的更新

这时，我们来更新 DaemonSet， 执行完了kubectl apply -f 后，它的 DaemonSet 就已经更新了。接下来我们去查看 DaemonSet 的更新状态。

![avatar](https://images.gitbook.cn/FtvHBGbpXhkp9Pq9PpJWmgGWrP5l)
...
![avatar](https://images.gitbook.cn/FqqpxDD5zywwJs1YPoFiBFtWN0D_)
...
![avatar](https://images.gitbook.cn/FlgZBztBwWxmJ2lFOPMD5NBnQZN5)
